  ...
  private async handleSimple(query: string, ws: WebSocket | null, state: AgentState) {
    ...
    try {
      ...
      try {
        ...
        const text = (await result.response).text?.() ?? 'Sorry, I could not respond.';
        full = text;
        if (ws) this.send(ws, { type: 'chunk', content: text });
      } catch (fallbackError) {
        console.error('Fallback failed:', fallbackError);
        full = 'Error: Unable to generate response.';
        if (ws) this.send(ws, { type: 'error', error: 'Response failed' });
      }
    }
...
Concise answer:`;
  ...
  // FIXED: Remove duplicate user messages
